...
const CSS = `
  ...
  .nwm { font-family: 'DM Sans', system-ui, sans-serif; }
  .nwm-card { background: rgba(255,255,255,0.025); border: 1px solid rgba(255,255,255,0.07); border-radius: 20px; padding: 24px; margin-bottom: 16px; position: relative; overflow: hidden; }
  .nwm-card::after { content:''; position:absolute; top:0; left:0; right:0; height:1px; background:linear-gradient(90deg,transparent,rgba(255,255,255,0.08),transparent); }
  .nwm-input { width:100%; background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.09); border-radius:11px; padding:10px 14px; color:#e2e8f0; font-size:13px; outline:none; font-family:inherit; transition:border-color 0.2s; }
  .nwm-input:focus { border-color:rgba(167,139,250,0.4); }
  .nwm-input::placeholder { color:rgba(100,116,139,0.5); }
  .nwm-select { width:100%; background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.09); border-radius:11px; padding:10px 14px; color:#e2e8f0; font-size:13px; outline:none; font-family:inherit; cursor:pointer; }
  .nwm-select:focus { border-color:rgba(167,139,250,0.4); }
  .nwm-select option { background:#0e1018; color:#e2e8f0; }
  ...
  .nwm-row { display:flex; align-items:center; justify-content:space-between; padding:13px 16px; border-radius:12px; background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.06); transition:all 0.2s; margin-bottom:8px; }
  .nwm-row:hover { background:rgba(255,255,255,0.05); border-color:rgba(255,255,255,0.1); }
...
// Allowed asset types for this page
const ASSET_TYPES = [
  "Real Estate",
  "Property",
  "Savings Account",
    ...
    <>
      ...
      <div className="nwm">
        ...
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:24 }}>
          ...
          <div>
            <div style={{ fontSize:10, fontWeight:700, color:"rgba(148,163,184,0.5)", letterSpacing:"0.1em" }}>WEALTH MANAGEMENT</div>
        ...
        <div className="nwm-stat-grid">
          <div className="stat-card">
            ...
            <div style={{ fontSize:32, fontWeight:800, color:"#fff", letterSpacing:"-0.04em", fontVariantNumeric:"tabular-nums" }}>
            ...
            <div style={{ marginTop:12, display:"flex", alignItems:"center", gap:8 }}>
              ...
              <span style={{ fontSize:10, color:"rgba(148,163,184,0.5)", whiteSpace:"nowrap" }}>{assetPct}% assets</span>
            </div>
          </div>
        ...
        <div className="nwm-card">
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:18 }}>
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              ...
              <div>
                ...
                <h2 style={{ fontSize:15, fontWeight:700, color:"#fff", margin:0 }}>Assets</h2>
              </div>
          ...
          {addingAsset && (
            <div style={{ background:"rgba(74,222,128,0.05)", border:"1px solid rgba(74,222,128,0.15)", borderRadius:14, padding:16, marginBottom:16 }}>
              ...
              <div style={{ display:"flex", gap:8 }}>
                ...
                <button className="nwm-btn" style={{ background:"rgba(255,255,255,0.06)", color:"rgba(148,163,184,0.7)" }} onClick={()=>setAddingAsset(false)}>
                  <X size={13}/> Cancel
                </button>
              </div>
            </div>
          ...
          {displayAssets.length === 0 && !addingAsset && portfolioCount === 0 && (
            <div style={{ textAlign:"center", padding:"28px 0", color:"rgba(148,163,184,0.35)", fontSize:13 }}>
              ...
              {editingAssetId === asset.id ? (
                <div style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.09)", borderRadius:12, padding:14, marginBottom:8 }}>
                  ...
                  <div style={{ display:"flex", gap:8 }}>
                    ...
                    <button className="nwm-btn" style={{ background:"rgba(255,255,255,0.06)", color:"rgba(148,163,184,0.7)" }} onClick={()=>setEditingAssetId(null)}>
                      <X size={13}/> Cancel
                    </button>
                  </div>
                </div>
                ...
                <div className="nwm-row">
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                    ...
                    <button className="nwm-icon-btn" style={{ background:"rgba(34,211,238,0.1)", color:"#22d3ee" }}
                      onClick={()=>{ setEditingAssetId(asset.id); setEditName(asset.name); setEditAmount(asset.amount); setEditType(asset.type); }}>
                      <Edit2 size={12}/>
                    </button>
          ...
          {legacyInvestmentAssets.map((asset) => (
            <div key={asset.id} className="nwm-row" style={{ borderColor:"rgba(251,191,36,0.15)", background:"rgba(251,191,36,0.03)", opacity:0.7 }}>
              <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                ...
                <div>
                  <p style={{ fontSize:13, fontWeight:600, color:"rgba(148,163,184,0.6)", margin:0 }}>{asset.name}</p>
                  <p style={{ fontSize:11, color:"rgba(251,191,36,0.5)", margin:"2px 0 0" }}>{asset.type} · excluded from total (move to Investments)</p>
                </div>
              </div>
              <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                <span style={{ fontSize:14, fontWeight:700, color:"rgba(148,163,184,0.4)", fontVariantNumeric:"tabular-nums", textDecoration:"line-through" }}>
                  ₹{asset.amount?.toLocaleString("en-IN")}
                </span>
                <button className="nwm-icon-btn" style={{ background:"rgba(248,113,113,0.1)", color:"#f87171" }} onClick={()=>deleteAsset(asset.id)}>
                  <Trash2 size={12}/>
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:18 }}>
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              ...
              <div>
                ...
                <h2 style={{ fontSize:15, fontWeight:700, color:"#fff", margin:0 }}>Liabilities</h2>
              </div>
          ...
          {addingLiability && (
            <div style={{ background:"rgba(248,113,113,0.05)", border:"1px solid rgba(248,113,113,0.15)", borderRadius:14, padding:16, marginBottom:16 }}>
              ...
              <div style={{ display:"flex", gap:8 }}>
                ...
                <button className="nwm-btn" style={{ background:"rgba(255,255,255,0.06)", color:"rgba(148,163,184,0.7)" }} onClick={()=>setAddingLiability(false)}>
                  <X size={13}/> Cancel
                </button>
              </div>
            </div>
